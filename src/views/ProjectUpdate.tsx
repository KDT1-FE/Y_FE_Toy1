...
import { WriteType } from './ProjectWirte';
...
const ProjectUpdate = () => {
  ...
  if(!isLoading){
    return (
      ...
      <div>
        {updateData.writeDate}
      </div>
      <div className="write__radios-box">
        <div className="write__radio-box">
            <input type='radio' id='expected' value={'expected'} name='status' onChange={changeRadioHandle} checked={updateData.status === 'expected'} />
            <label htmlFor='expected'>예정</label>
        </div>
        <div className="write__radio-box">
            <input type='radio' id='progress' value={'proceeding'} name='status' onChange={changeRadioHandle} checked={updateData.status === 'proceeding'} />
            <label htmlFor='progress'>진행중</label>
        </div>
        <div className="write__radio-box">
            <input type='radio' id='complete' value={'complete'} name='status' onChange={changeRadioHandle} checked={updateData.status === 'complete'} />
            <label htmlFor='complete'>완료</label>
        </div>
        </div>
        <input className="write__input-title" name='title' placeholder="제목을 입력해주세요" value={updateData.title} onChange={changeHandle} />
        <Editor
          ref={editorRef}
